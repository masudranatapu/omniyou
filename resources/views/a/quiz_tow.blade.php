        ...
        <div class="box_wrapper question_ans" style="background: black">
            ...
            <form action="{{route('start-quiz-answer')}}" method="post">
                ...
                <div class="timer_seconds">
                 ...
                 @php
                     $answer = App\UserQuizAnswer::where('interview_user_id',Session::get('interview_user_id'))->where('quiz_question_id',$item->id)->first();
                 @endphp
                 <div class="row g-4">
                     ...
                     <div class="col-lg-6">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" value="{{$ques->id}}" name="quiz_answer_id" id="ans{{$ques->id}}"  required>
                            <input type="hidden" value="{{$step}}" name="step_id">
                            <label class="form-check-label" for="ans{{$ques->id}}" style="color: #fff">{{$ques->answer_option}}</label>
                        </div>
                    </div>
                     @endforeach
                 </div>
