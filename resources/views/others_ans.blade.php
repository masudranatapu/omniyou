        ...
        <div class="box_wrapper question_ans">
            ...
            <form action="{{route('free-writing-ans')}}" method="post">
                ...
                @php
                     ...
                     $others_ans = App\UserFreeWritingAns::where('interview_user_id','!=',Session::get('interview_user_id'))->where('quiz_question_id',$item->id)->OrderBy('id','desc')->get();
                ...
                <div class="row g-4 mb-4">
                    <div class="col-lg-12">
                        <div class="form-check">
                            ...
                            @php
                                $o_answer = App\UserFreeWritingAns::where('interview_user_id',$ans->interview_user_id)->where('quiz_question_id',$item->id)->first();
                            @endphp
                                <span style="color:blue">{{$ans->interview_user->name ?? ''}}</span><br/> <span>Ans :{!! $o_answer->answer ?? '' !!}</span><br />
                            @endforeach
