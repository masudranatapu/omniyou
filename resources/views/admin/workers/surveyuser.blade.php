...
@section('content')
    @php
        $servey = DB::table('survey')
            ->where('id', $workers->running_survey_id)
            ->first();
    @endphp
    <div class="row">
        <div class="col-12">
                        ...
                        <div class="col-lg-12">
                            ...
                            @foreach ($clients as $client)
                                <label for="cl_lbl_{{ $client->id }}" class="pr-5" style="cursor: pointer">
                                    <input type="checkbox" class="checkbox" name="client" id="cl_lbl_{{ $client->id }}"
                                        @if (in_array($client->id, $client_survey)) checked @endif
                                        data-client_id="{{ $client->id }}" data-survey_id="{{ $servey->id }}"
                                        data-worker_id="{{ $workers->id }}" /> {{ $client->name }}
                                </label>
                            @endforeach
                        </div>
                    </div>
